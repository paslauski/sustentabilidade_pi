...
while programaAtivo:
    ...
    ╚══════════════════════════════════════════════════╝ """)
            ...
            ID: {linha[0]}
            \t Data: {linha[1]}
            \t Consumo de água: (L) {linha[2]}
            \t Resíduos não recicláveis: (kg) {linha[3]}
            \t Resíduos reciclados: (%) {linha[4]}
            \t Energia gasta: (kW) {linha[5]}
            \t Utilizou carro? {linha[6]}
            \t Pegou carona? {linha[7]}
            \t Utilizou bicicleta? {linha[8]}
            \t Utilizou transporte público? {linha[9]}
            \t Utilizou carro elétrico? {linha[10]}
            \t Caminhou? {linha[11]}""") 
            pausar() 
            print("\nTABELA RESULTADOS: ")
            ...
            \t Data: {linha[1]}
            \t Uso de água: {linha[2]}
            \t Reciclagem de resíduos: {linha[3]}
            \t Uso de eletricidade: {linha[4]}
            \t Uso de transportes: {linha[5]}""")
            pausar()       
        case 5: #LISTAR MÉDIA\n
            print("""
    ╔══════════════════════════════════════════════════╗
    ...
    ╚══════════════════════════════════════════════════╝ """)
            ...
            for linha in sustentabilidade:
                ...
                num_registro += 1
            resultado = cursor.fetchall()     
            print(f"\n\tMÉDIAS DE REGISTROS:\n\tÁgua utilizada: (L) {soma_ca / num_registro : .1f}\n\tResíduos não recicláveis: (kg) {soma_nrq / num_registro : .1f}\n\tResíduos reciclados: (%) {soma_nrp / num_registro : .1f}\n\tEnergia consumida: (kW) {soma_ce / num_registro : .1f}")
            pausar() 
        case 6:
            programaAtivo = False
            cursor.close()
